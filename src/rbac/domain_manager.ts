...
export class DomainManager implements IDefaultRoleManager {
    ...
    constructor(maxHierarchyLevel: number = 10, domainMatcher?: IPatternMatcher, roleMatcher?: IPatternMatcher ) {
    ...
    getRoleManager(domain: string, store: boolean): IDefaultRoleManager {
        ...
        if (domain != defaultDomain) {
            rm = new DomainManager(this.maxHierarchyLevel - 1, this.domainMatcher, this.roleMatcher)
        } else {
    ...
    eachLinkHelper(rmMap: Map<string, IDefaultRoleManager>, cb: (user:string, role:string, ...domain: string[]) => void) {

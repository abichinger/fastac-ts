...
export class RoleManager implements IDefaultRoleManager {
  ...
  constructor(
    ...
    matcher: IPatternMatcher | undefined = undefined
  ) {
    ...
    for (let role of roles) {
      if (
        ...
        (this.matcher !== undefined &&
          this.matcher.match(role.name, this.matcher.parse(target)))
      ) {
        return true;
      }
      role.eachRole(r => {
  ...
  hasLink(user: string, role: string): boolean {
    if (
      user === role ||
      (this.matcher !== undefined &&
        this.matcher.match(user, this.matcher.parse(role)))
    ) {
      return true;
    }

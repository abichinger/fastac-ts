...
import { IDefaultRoleManager, IRoleManager } from "../rbac/rbac_api";
...
export abstract class DefHandler<T> {
    ...
    prop(key:string, name: string, value: string) {
        ...
        if (propHandler === undefined) {
            throw new Error(`no handler found for property '${name}' (${key}.${name})`)
        }
        let instance = this.get(key)
        if (instance === undefined) {
            throw new Error(`can't set property ${name} of undefined (${key}.${name})`)
        }
...
export class RoleManagerHandler extends DefHandler<RolePolicy> {
    ...
    constructor(){
        ...
        this.registerProp("roleMatcher", (rolePolicy, value) => {
            ...
            if (matcher === undefined) {
                throw new Error(`PatternMatcher '${value}' not found, use setPatternMatcher to register a new matcher`)
            }
